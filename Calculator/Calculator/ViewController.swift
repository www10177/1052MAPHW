...
class ViewController: UIViewController {
    ...
    @IBAction func changeSign(_ sender: UIButton) {
        let text = self.displayLabel.text ?? "0"
        if text == "0"{
        }
        else if text[text.startIndex] == "-"{
            let start = text.index(text.startIndex,offsetBy:1)
            let end=text.endIndex
            self.displayLabel.text=text.substring(with:start..<end)
        }
        else{
    ...
    @IBAction func percent(_ sender: UIButton) {
        let result = Double(self.displayLabel.text ?? "0")! / 100
        self.displayLabel.text = result.displayString
    }
    @IBAction func pi(_ sender: UIButton) {
        self.displayLabel.text=Double.pi.displayString
    }
    @IBAction func eulerNumberClicked(_ sender: UIButton) {
        self.displayLabel.text=2.71828182846.displayString
    }

    @IBAction func logClicked(_ sender: UIButton) {
        var num = Double(self.displayLabel.text ?? "0")!
        num = log10(num)
        self.displayLabel.text=num.displayString
    }
    ...
    @IBAction func operatorButtonClicked(_ sender: UIButton) {
        ...
        case 1104: //Divide
            try! self.core.addStep(/)
        case 1105: //power
            try! self.core.addStep({x, y in return pow(x,y)})
        case 1106: // root
            try! self.core.addStep({x,y in return pow(x,1/y)})
        default:
